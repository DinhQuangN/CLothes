<template>
  <div class="w-full absolute bottom-0 left-0 bg-white text-[#080710] rounded-[10px] z-10 list" v-if="handleOpen()">
    ...
    <ul class="w-full h-[250px] overflow-y-scroll" >
        <li class="w-full py-[10px] px-[30px] flex justify-between cursor-pointer border-b border-[#eee] hover:bg-[palegoldenrod]" v-for="(item,index) in handleData()" :key="index" :class="{playing:handleMusicNumber() === index }" @click="handleClickMusicNumber(index)">
            <div>
